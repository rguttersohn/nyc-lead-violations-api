...
trait ViolationQueries {
    ...
    private function queryDistrictViolations(string $table_name, string $uri, string $status, string $start_year, string $end_year, bool $status_needs_checking = false){
      ...
      return DB::table("$table_name")
            ->selectRaw("'$start_year' as start_year,'$end_year' as end_year, '$status' as status, '$district_selection_name' as district_type, $housing_table.units as total_housing_units, $table_name.$district_selection_name as district, $table_name.geo_type, st_asgeojson(polygon) as polygon, st_asgeojson(multipolygon) multipolygon, COALESCE(COUNT(v.nyc_open_data_violation_id),0) as violations, COUNT(b.*) FILTER(WHERE v.building_id = b.nyc_open_data_building_id) as buildings_with_violations, COUNT(DISTINCT(v.apartment,v.building_id)) FILTER(WHERE v.building_id = b.nyc_open_data_building_id) as units_with_violations")
    ...
    private function getDistrictViolationsWithID(string $table_name, string $district_id, string $uri, string $status, string $start_year, string $end_year, bool $status_needs_checking = false){
      ...
      $housing_table = $this->formatDistrictHousingTableName($table_name);
      return DB::table("$table_name")
        ->selectRaw("'$start_year' as start_year,'$end_year' as end_year, $housing_table.units as total_housing_units, '$status' as status, $table_name.$district_selection_name as district, COALESCE(COUNT(v.*),0) as violations, COUNT(DISTINCT b.nyc_open_data_building_id) FILTER(WHERE v.building_id = b.nyc_open_data_building_id) as buildings_with_violations, COUNT(DISTINCT(v.apartment,v.building_id)) FILTER(WHERE v.building_id = b.nyc_open_data_building_id) as units_with_violations")
